...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason="Waiting for reviewer escalation answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason="Answered escalation questions",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
def handle_interrupt(payload):
    ...
    if kind == "approval_request":
        console.print(Panel.fit(payload["summary"], title=f"conf={payload['confidence']:.0%}", border_style="green"))
